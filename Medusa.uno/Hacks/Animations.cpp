...
void Animations::update(UserCmd* cmd, bool& _sendPacket) noexcept
{
    ...
    static std::array<AnimationLayer, 13> cachedLayers{};
    static float cachedAlpha = 1.0f;
    static bool cachedSwitchAlpha = false;
    static float alpha = 1.0f;
    static bool switch_alpha = false;
    ...
    if (!isCached || spawnTime != localPlayer->spawnTime())
    {
    ...
    {
        if (!isCached || cachedAlpha <= 0.0f || cachedAlpha >= 1.0f)
            cachedSwitchAlpha = !cachedSwitchAlpha;

        cachedAlpha += cachedSwitchAlpha ? 1.75f * memory->globalVars->frametime : -1.75f * memory->globalVars->frametime;
        cachedAlpha = std::clamp(cachedAlpha, 0.0f, 1.0f);

        if (localPlayer->flags() & 1)
        {
            if (cachedSwitchAlpha)
            {
                localPlayer->setPoseParameter(-1, 0);
                localPlayer->setPoseParameter(1, 1);
            }
            else if (!cachedSwitchAlpha)
            {
                localPlayer->setPoseParameter(1, 0);
                localPlayer->setPoseParameter(-1, 1);
            }
    ...
    // Cache the animation layers
    std::memcpy(&cachedLayers, localPlayer->animOverlays(), sizeof(AnimationLayer) * localPlayer->getAnimationLayersCount());

    // Cache the alpha and switch_alpha values
    cachedAlpha = alpha;
    cachedSwitchAlpha = switch_alpha;
...
void Animations::handlePlayers(FrameStage stage) noexcept {
    ...
    const int maxClients = interfaces->engine->getMaxClients();
    ...
    for (int i = 1; i <= maxClients; ++i) {
        ...
        auto frameTimeBackup = memory->globalVars->frametime;
        auto currentTimeBackup = memory->globalVars->currenttime;

        memory->globalVars->frametime = memory->globalVars->intervalPerTick;
        memory->globalVars->currenttime = entity->simulationTime();

        auto effectsBackup = entity->getEffects();
        entity->getEffects() |= 8;
        ...
        if (player.simulationTime != entity->simulationTime() && player.simulationTime < entity->simulationTime()) {
            ...
            const auto simDifference = std::fabs(entity->simulationTime() - player.simulationTime);
            ...
            if (entity->flags() & 1) {
                ...
                if (weight > 0.0f) {
                    const float prevRate = player.oldlayers[ANIMATION_LAYER_ALIVELOOP].playbackRate;
                    const float currRate = player.layers[ANIMATION_LAYER_ALIVELOOP].playbackRate;
                    if (prevRate == currRate) {
                        const int prevSequence = player.oldlayers[ANIMATION_LAYER_ALIVELOOP].sequence;
                        const int currSequence = player.layers[ANIMATION_LAYER_ALIVELOOP].sequence;
            ...
            if (entity->flags() & 1 && player.layers[ANIMATION_LAYER_ALIVELOOP].weight > 0.f &&
            ...
            if ((player.layers[ANIMATION_LAYER_MOVEMENT_MOVE].playbackRate <= 0.f ||
                player.layers[ANIMATION_LAYER_MOVEMENT_MOVE].weight <= 0.f) && (entity->flags() & 1)) {
        ...
        memory->globalVars->frametime = frameTimeBackup;
        memory->globalVars->currenttime = currentTimeBackup;
        entity->getEffects() = effectsBackup;
