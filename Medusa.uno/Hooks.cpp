...
static bool __fastcall writeUsercmdDeltaToBuffer(void* thisPointer, void* edx, int slot, bufferWrite* buffer, int from, int to, bool newCmd) noexcept
{
    ...
    auto next_cmd_nr = memory->clientState->lastOutgoingCommand + memory->clientState->chokedCommands + 1;

    moveMsg->new_commands = std::clamp(moveMsg->new_commands + Tickbase::getTickshift(), 1, 15);

    const int extraCommands = Tickbase::getTickshift();
    Tickbase::resetTickshift();

    for (to = next_cmd_nr - new_commands + 1; to <= new_commands; to++)
    {
